...
class UGF(object):
    def __init__(self, data_loader, k, eval_metric_list, fairness_metric='f1',
        ...
        self.dataset_name = data_loader.path.split('/')[-2]
        self.k = k
...
if __name__ == '__main__':
    ...
    epsilon = 0.05                           # fairness constraint coefficient
    dataset_folder = './result'           # dataset directory
    dataset_name = 'Epinion'           # dataset name
    model_name = 'NeuMF'                      # model name (which model does this ranking file generated by)
    group_name_title = 'count_0.05'           # grouping method name for distinguish different experiment results
    logger_dir = os.path.join(dataset_folder, dataset_name)    # logging file path
    logger_dir = os.path.join(logger_dir, model_name)

    data_path = os.path.join(dataset_folder, dataset_name)
    data_path = os.path.join(data_path, model_name)
    rank_file = model_name + '_best_ndcg_active_rank.csv'                                    # original input ranking csv file name
    group_1_file = group_name_title + '_active_test_ratings.txt'      # advantaged group testing file name
    group_2_file = group_name_title + '_inactive_test_ratings.txt'    # disadvantaged group testing file name
    ############################################################################
